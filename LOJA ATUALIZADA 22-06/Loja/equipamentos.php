...
$dbname = "produtos";
 try {
	    ...
	    // forma 1
		$select = $conn->query("SELECT * FROM produtos where CategoriaProduto='Equipamentos' ");
...
$result_produtos = "SELECT * FROM produtos where CategoriaProduto='Equipamentos' ";
...
$result_produtos = "SELECT * FROM produtos where CategoriaProduto='Equipamentos' limit $incio, $quantidade_pg";
$resultado_produtos = mysqli_query($conn, $result_produtos);
$total_produtos = mysqli_num_rows($resultado_produtos);
?>
...
<html>
    ...
    <button onclick="topFunction()" id="myBtn" title="Go to top"><i class="up"></i></button>  
       ...
       <div class="page-header text-center">
           <h1>Categoria : Equipamentos</h1>
       </div>
        <div class="row">
             <?php while($row = mysqli_fetch_assoc($resultado_produtos)){ ?>
              <div class="col-sm-6 col-md-4">
                <div class="thumbnail">
                    <div class="nomeproduto">
                        <h3><?php echo $row['NomeProduto']; ?></h3>
                    </div>
                        <div class="generoprod">
                            <h6></h6><?php echo $row['CategoriaProduto']; ?><h6>
                        </div>
                         <img class="imgproduto" src="imagens/<?php echo $row['ImagemProduto'];?>"  alt="lascou">
                          <div class="caption text-center">
                            <p>A Partir de : <br><mark>R$<?php echo $row['PrecoProduto'];?></mark></p>
				...
				<ul class="pagination">
					<li>
						...
						if($pagina_anterior != 0){ ?>
							<a href="equipamentos.php?pagina=<?php echo $pagina_anterior; ?>" aria-label="Previous">
					...
					for($i = 1; $i < $num_pagina + 1; $i++){ ?>
						<li><a href="equipamentos.php?pagina=<?php echo $i; ?>"><?php echo $i; ?></a></li>
					<?php } ?>
					<li>
						<?php
						if($pagina_posterior <= $num_pagina){ ?>
							<a href="equipamentos.php?pagina=<?php echo $pagina_posterior; ?>" aria-label="Previous">
